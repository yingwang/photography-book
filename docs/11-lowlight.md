...
前面反复说读出噪声在弱光里最要命，那条几乎固定的地板决定了暗部什么时候开始脏。顺着这条线索，就能引出一个许多人没有留意、却相当实用的机身特性：双原生 ISO。传统的经验里，感光度和画质是一条单调下滑的曲线，ISO 越低越干净，一路往上噪点越多，于是弱光里人人都想着能压多低就压多低。可不少近些年的机型并不遵守这条曲线，它们的传感器在读出电路上做了两套放大增益，行为于是变得和直觉不太一样。

所谓**双原生 ISO**（dual native ISO，也叫 dual conversion gain，双转换增益），指的是传感器在某一档感光度上会切换到第二套读出电路，这套电路专为高感光度优化，把读出噪声压得更低。因此这类机身实际上有两个原生的起点：一个在最低感光度附近，另一个则落在中高感光度的某一档，常见的位置在 ISO 2500 或 ISO 4000 一带，具体数值随机型而异。它带来的推论和老经验正好相反：在切换点以下，画质随 ISO 升高而缓慢变差；可一旦跨过那道门槛、进入第二个原生 ISO，读出噪声会突然回落，画质不降反升。也就是说，当你需要的曝光正好落在切换点稍下方，比如 ISO 1600，有时把感光度直接推到第二个原生点 ISO 2500，画面反而更干净。
...
式子里 \(N\) 是光圈的 f 值，\(p\) 是传感器的像素间距、以微米为单位，\(f\) 是镜头的实际焦距、以毫米计，算出来的 \(t\) 就是星点还能保持锐利的曝光秒数。和 500 法则相比，它多顾及了两件事：光圈越大、进光越猛，星点越容易在很短的时间里就显出移动，所以 \(N\) 落在分子上；像素越密、单个像素越小，同样一点点位移就越容易被记成一道短线，所以 \(p\) 也落在分子上。正因为把这两样都算了进去，NPF 法则给出的时间通常比 500 法则更短、也更保守。以一台约 2400 万像素的全画幅、像素间距约 5.9 微米为例，两条法则算出来的差别相当明显：
...
500 法则也好，NPF 法则也好，说到底都是在想方设法不让星星拖线。可换一个方向想，星星拖出来的那道线，本身也可以是主题，这就是星轨。地球自转让所有的星星绕着天极缓缓打转，只要曝光足够长，它们划过的弧线就会一圈圈叠在一起，在画面上绕着北极星（在北半球）转出一组同心圆。拍星轨有两条路。一条是索性开一次极长的曝光，让快门连开几十分钟，把整段轨迹一次记下来；可这么长的时间里，暗电流噪声和热噪声会一路累积，画面很难保持干净。另一条更稳妥的路，是把这几十分钟拆成许多张各自几十秒的照片连续拍下来，回到后期再用叠加的方式把它们摞在一起，既接出了完整的长弧线，又避开了单张超长曝光带来的噪点。同是面对地球自转，星空摄影竭力想躲开它，星轨摄影却反过来，把它当成了自己的画笔。
